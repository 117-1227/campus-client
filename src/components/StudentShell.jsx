...
export default function StudentShell({ auth, onLogout, remainMs, formatRemain, expiredModal }) {
  ...
  const renderPage = () => { switch (currentPage) { case 'clockInOut': return <ClockInOut />; case 'profile': return <Profile />; case 'workHoursRecord': return <WorkHoursRecord />; case 'attendanceCorrection': return <AttendanceCorrection />; default: return <ClockInOut /> } }

  return (
    <div className="flex h-screen bg-[#f3f4f6]">
      <aside className="w-60 bg-emerald-600 text-white flex flex-col shrink-0">
        ...
        <nav className="flex-1 py-4 px-3 space-y-1">
          {PAGES.map(p => { const active = currentPage === p.key; return (
            <button key={p.key} onClick={() => setCurrentPage(p.key)} className={'w-full flex items-center gap-3 px-4 py-3 text-sm rounded-lg transition-all duration-150 ' + (active ? 'bg-white/15 text-white font-medium' : 'text-emerald-100 hover:text-white hover:bg-white/10')}>
              <span className={active ? 'text-white' : 'text-emerald-200'}>{p.icon}</span><span>{p.label}</span>
            </button>
          )})}
        </nav>
        <div className="px-6 py-4 border-t border-white/10 space-y-3">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0"><p className="text-sm text-white font-medium truncate">{auth.user?.username || 'student'}</p><p className="text-xs text-emerald-200">学生</p></div>
            <button onClick={onLogout} className="ml-auto p-2 text-emerald-200 hover:text-white hover:bg-white/10 rounded-lg transition-colors" title="退出"><svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9" /></svg></button>
          </div>
          <div className="flex items-center justify-between"><p className="text-xs text-emerald-200">v1.0</p>{remainMs > 0 && <p className="text-xs text-emerald-200">剩余 {formatRemain(remainMs)}</p>}</div>
        </div>
      </aside>
